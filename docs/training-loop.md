...
`GET /api/coaches/{coach_id}/dashboard` 仅允许教练本人访问。汇总有效关系中的球员、当前授权视频、评审、训练任务、球员反馈和复测。所有计数遵守与详情相同的权限：无逐视频许可时只展示已建立关系的球员名称；撤销关系则移除球员。删除/撤权的视频不参与任务或评审统计，复测要求两段视频都可访问。待办按提交时间从早到晚显示，训练进展按任务更新时间显示，不作为能力评分。无需新增数据库迁移。

验证：后端完整 53 项测试通过；前端 build/lint 通过（仍有非阻断警告）。隔离浏览器验证了教练登录路由、球员选择、任务/反馈展示和进入训练计划；球员首页继续展示本人视频。
